...
test('students enter automatically; classroom identity and period boundaries own the room', () => {
  ...
  const join = socket => service.handle(socket, {type:'park_join',clientId:'browser_one',section:'forged',member:'forged'});
  ...
  assert.ok(!sent.some(row=>row.ws===outsider&&row.message.kind==='contribution'));
  ...
  const resumed=service.handle(replacement,{type:'park_resume',clientId:'browser_one',epoch:a.epoch,since:a.revision});
...
test('unjoined sockets, invalid clients, stale bindings and section changes are handled', () => {
  ...
  assert.equal(service.handle(socket,{type:'park_join',clientId:'browser_one'}).type,'park_error');
  registry.join(socket,'B','alice','student',0);
  assert.equal(service.handle(socket,{type:'park_join',clientId:'bad'}).type,'park_error');
  const first=service.handle(socket,{type:'park_join',clientId:'browser_one'});
  ...
  const next=service.handle(socket,{type:'park_join',clientId:'browser_one'});
  assert.notEqual(next.epoch,first.epoch);
  service.close();
});

test('an outage across the hour preserves an attempt; an explicit last exit permits rotation', () => {
  let wall = 0;
  const registry = createClassroomRegistry(), socket = {};
  registry.join(socket, 'B', 'alice', 'student', 0);
  const service = createParkService({ registry, wallNow: () => wall, send() {} });
  const join = () => service.handle(socket, { type: 'park_join', clientId: 'browser_one' });
  ...
  assert.equal(join().level.index, 1);
  service.close();
});
