...
test('presence survives missed events, idle time and overlapping reconnect sockets', () => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    service.handle(alice, { ...join, type: 'park_resume', epoch: first.epoch, since: replica.revision });
  }
  ...
  const recovered = service.handle(replacement, { ...join, type: 'park_resume', epoch: first.epoch, since: replica.revision });
  ...
  service.handle(bob, { type: 'park_join', clientId: 'browser_b' });
  replica.resume(service.handle(replacement, { ...join, type: 'park_resume', epoch: first.epoch, since: replica.revision }));
  assert.deepEqual(replica.state.online, ['alice', 'bob']);
  service.close();
});
