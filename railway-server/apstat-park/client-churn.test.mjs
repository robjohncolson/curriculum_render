...
test('closed tabs can be replaced repeatedly; reclaimed clients safely retry saved and pending actions', () => {
  ...
  const join = id => service.handle(student, { type: 'park_join', protocol: 4, clientId: id });
...
test('four active tabs retain their slots; leaving one permits a replacement', () => {
  ...
  const buddy = {}; registry.join(buddy, 'a', 'bob', 'student', 0);
  service.handle(buddy, {type:'park_join',protocol:4,clientId:'buddy_browser'});
  const join = i => service.handle(sockets[i], { type: 'park_join', protocol: 4, clientId: `browser_${i}` });
